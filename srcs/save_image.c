...
/*   Updated: 2020/01/20 15:10:13 by jjosephi         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	convert_rgb(t_data **data)
{
	...
	{
		(*data)->image.img_adress[i] =
		((*data)->image.img_adress[i] & 255) << 16 |
...
void	save_file(t_data **data)
{
	...
	convert_rgb(data);
	stbi_write_jpg(str, (*data)->w, (*data)->w, 4,
	(*data)->image.img_adress, 500);
	free(filename);
	free(str);
}
